...
import com.yuanqi.hangzhou.imhookup.utils.MapHelper;
import com.yuanqi.hangzhou.imhookup.utils.NimLocationManager;
import com.yuanqi.hangzhou.imhookup.utils.StringUtil;

import java.util.ArrayList;
import java.util.List;

public class NavigationAmapActivity extends BaseActivity implements LocationExtras, NimLocationManager.NimLocationListener,
...
//        setToolBar(R.id.toolbar, options);
        setToolbar(this,0,"地图");
        onInitRightSure(this, "导航", new onToolBarRightTextListener() {
            @Override
...
//            setTitle(R.string.location_map);
    ...
    private void showLocationFailTip() {
        if (firstLocation && firstTipLocation) {
            ...
            myAddressInfo = "该位置信息暂无";
            ToastHelper.showToast(this, "获取位置信息失败");
        }
    }
    ...
    private void doNavigate(final NimLocation origin, final NimLocation des) {
        ...
        if (infos.size() >= 1) {
            ...
            dialog.setTitle("选择工具");
            dialog.show();
        } else {
            IconListAdapter.IconListItem item = new IconListAdapter.IconListItem("在网页中打卡", null, null);
            items.add(item);
            CustomAlertDialog dialog = new CustomAlertDialog(this, items.size());
            dialog.setAdapter(adapter, new DialogInterface.OnClickListener() {
                @Override
