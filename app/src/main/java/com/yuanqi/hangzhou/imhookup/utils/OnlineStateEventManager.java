...
public class OnlineStateEventManager {
    ...
    private static ContactChangedObserver observer = new ContactChangedObserver() {
        ...
        public void onAddedOrUpdatedFriends(List<String> accounts) {
            ...
            Log.e("OnlineStateEventManager","added or updated friends subscribe online state " + subs);
    ...
    private static String getMobileOnlineClientString(Context context, OnlineState state, boolean ios, boolean simple) {
        ...
        if (!validNetType(state)) {
            ...
            if (simple) {
                // 简单展示
                result = getDisplayNetState(state.getNetState()) +"在线";
            } else {
                // 详细展示
                result = client + " - " + getDisplayNetState(state.getNetState()) + "在线";
            }
        }
        return result;
    }
    ...
    private static boolean enableOnlineStateEvent() {
        ...
        return enable = (packageName != null && packageName.equals("com.yuanqi.hangzhou.imhookup"));
    }
}